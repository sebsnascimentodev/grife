...
router.get('/', requireAdmin, async (req, res) => {
  ...
  res.json(db.cupons.map((c) => ({ ...c, status: cupomStatus(c) })));
});
...
router.post('/validar', async (req, res) => {
  ...
  const db = await readDb();
  const cupom = db.cupons.find((c) => c.codigo.toLowerCase() === String(codigo || '').toLowerCase());
...
router.post('/', requireAdmin, async (req, res) => {
  ...
  const db = await readDb();
  if (db.cupons.some((c) => c.codigo.toLowerCase() === codigo.toLowerCase())) {
...
router.put('/:codigo', requireAdmin, async (req, res) => {
  const db = await readDb();
  const cupom = db.cupons.find((c) => c.codigo === req.params.codigo);
  if (!cupom) return res.status(404).json({ erro: 'Cupom não encontrado' });
...
router.delete('/:codigo', requireAdmin, async (req, res) => {
  const db = await readDb();
  const idx = db.cupons.findIndex((c) => c.codigo === req.params.codigo);
